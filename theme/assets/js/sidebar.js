document.addEventListener("DOMContentLoaded", function () {
  ...
  function findToggleLinkForCollapse(collapseEl) {
    ...
    if (sibling && sibling.matches('a[data-bs-toggle="collapse"]')) {
      ...
      if (href === "#" + collapseEl.id || target === "#" + collapseEl.id)
        return sibling;
    }

    var parent = collapseEl.parentElement;
    if (parent) {
      var inSameItem = parent.querySelector(
        `a[data-bs-toggle="collapse"][href="#${collapseEl.id}"], a[data-bs-toggle="collapse"][data-bs-target="#${collapseEl.id}"]`,
      );
  ...
  collapseElements.forEach(function (collapseEl) {
    ...
    var folderIconClosed = navLink
      ? navLink.querySelector(".sidebar-folder-icon-closed")
      : null;
    var folderIconOpen = navLink
      ? navLink.querySelector(".sidebar-folder-icon-open")
      : null;
    console.log(
    ...
    if (!folderIconClosed || !folderIconOpen) {
      console.warn(
        "    One or both folder icons not found for navLink:",
        navLink,
      );
      return;
    }
    ...
    if (collapseEl.classList.contains("show")) {
      ...
      console.log(
        "    Initial state: Collapse is HIDDEN, showing closed icon.",
      );
  ...
  if (activeLink) {
    ...
    while (parentCollapse) {
      ...
      if (!parentNavLink) {
        console.warn(
          "    Parent NavLink not found for parent collapse ID:",
          parentId,
        );
        break;
      }
      ...
      if (parentNavLink.classList.contains("nav-link")) {
        var parentFolderIconClosed = parentNavLink.querySelector(
          ".sidebar-folder-icon-closed",
        );
        var parentFolderIconOpen = parentNavLink.querySelector(
      ...
      parentCollapse = parentNavLink
        ? parentNavLink.closest(".collapse")
        : null;
    }
  } else {
  ...
  function loadCollapseStates() {
    const savedStates = JSON.parse(
      localStorage.getItem("sidebarCollapseStates"),
    );
    if (savedStates) {
      collapseElements.forEach(function (collapseEl) {
