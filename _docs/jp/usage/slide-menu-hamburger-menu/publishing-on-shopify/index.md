...
<details><summary>目的2: <strong>大きく柔軟なメニュー構造で無限のメニュー深度（Navi+のみ）</strong></summary>
...
<p>これを行うには、ウェブサイトのハンバーガーメニューアイコンのCSSセレクタを特定し、それをテキストボックスに入力します。私たちはすでに、以下にリストされた人気のあるテーマのいくつかのCSSセレクタを見つけました。</p>
<ol>
...
<pre class="codehilite"><code>- Navi+サポーターとチャットして即時のヘルプを受けることができます—CSSセレクタを見つけるのに数分しかかかりません。
...
<p>Navi+サポーターとチャットして即時のヘルプを受けることができますが、（1）これによりコアアイデアを理解するのに役立ち、（2）自分でやりたい場合は、以下の指示に従ってください。</p>
...
これはすぐに修正する必要があるものではありませんが、長期的にはNavi+によって作成されたスライドメニュー体験にとって重要です。Navi+が古いスライドメニューを完全に置き換える前に、テーマの元のメニューはまだアクティブです。短い間—約半秒—ユーザーがメニューを読み込み、ハンバーガーボタンを素早くタップした場合（稀なケースですが、可能性はあります）、古いテーマメニューが表示され、視覚的なグリッチを引き起こす可能性があります。

<details><summary>Navi+は、ユーザー体験と速度を最適化するための簡単な解決策を提供します。</summary>
<p><strong>なぜこれを行う必要があるのですか？Navi+はすでに速くないですか？</strong></p>
<p>Navi+は非常に速いです。非常に低いレイテンシ（約100ms）でリーディングCDNに完全に展開されており、数百万の顧客に対しても遅くなりません。</p>
<p>しかし、Navi+はまだShopifyエコシステム内で動作しています。Shopifyには独自の読み込みルールがあります：</p>
<ol>
...
<p>注意:</p>
<ul>
<li>モバイルのみに適用したい場合は、<strong>(M)</strong>の接尾辞を追加します。</li>
<li>デスクトップのみに適用したい場合は、<strong>(D)</strong>の接尾辞を追加します。</li>
<li>両方のプラットフォームに適用したい場合は、接尾辞を追加しないでください。</li>
</ul>
<p><strong>更新: インスタント読み込みスケルトンプレビュー</strong></p>
<p>ユーザーがNavi+まだ読み込み中にトリガーをタップするとすぐに、軽量なスケルトンドロワーが即座に表示されます — 目に見えるフィードバックなくタップを残すのではなく、アニメーション付きシマーバーを備えたプレースホルダーパネルです。Navi+の読み込みが完了した瞬間に自動的にフェードアウトし、実際のスライドメニューに引き継がれます。閉じるボタンまたは暗くなった背景をタップするとペンディング中のタップがキャンセルされるため、メニューが突然開くことはありません。</p>
<p>上記の<strong>(M)</strong>/<strong>(D)</strong>に使用される同じ括弧内に追加オプションを追加することで、スケルトンの外観をカスタマイズできます。<code>|</code>で区切ります:</p>
<pre class="codehilite"><code>.your-hamburger-selector (Device:M,D | Color:#1a1a1a | Direction:left | Width:85%)
</code></pre>
<ul>
<li><strong>Device</strong> — <code>M</code>、<code>D</code>、またはその両方(上のレガシーサフィックスと同じ意味)。デフォルト: 両方。</li>
<li><strong>Color</strong> — スケルトンの背景用の16進数カラー。例: <code>#1a1a1a</code>。閉じるアイコンとシマーバーは、明色と暗色の両方で読みやすくなるように自動調整されます。デフォルト: <code>#ffffff</code>。</li>
<li><strong>Direction</strong> — <code>left</code>または<code>right</code>。スケルトンがスライドインする側。デフォルト: <code>left</code>。</li>
<li><strong>Width</strong> — パーセンテージ(例: <code>85%</code>)または固定ピクセル値(例: <code>320px</code>)。デフォルト: <code>85%</code>。</li>
</ul>
<p>すべてのオプションはオプションであり、任意の順序で組み合わせることができます。スペーシングと大文字小文字は関係ありません — <code>color:#1A1A1A</code>と<code>Color: #1a1a1a</code>は同じように機能します。値が無効または入力間違いの場合、そのオプションは単にスキップされ(デフォルトにフォールバック)、同じセレクター内の他のオプションに影響を与えません。</p>
<p>上記で説明した単純な<code>(M)</code> / <code>(D)</code>構文は、以前とまったく同じように機能し続けます — これはただのオプションのアップグレードであり、置き換えではありません。</p>
</details>
