...
<details><summary>目的2: <strong>Navi+のみでの大きく柔軟なメニュー構造による無限のメニュー深度</strong></summary>
...
<p>これを行うには、ウェブサイトのハンバーガーメニューアイコンのCSSセレクタを特定し、テキストボックスに入力するだけです。私たちはすでに、いくつかの人気のあるテーマのCSSセレクタを見つけましたので、以下にリストします。</p>
<ol>
...
<pre class="codehilite"><code>- Navi+サポーターとチャットして、瞬時に助けを得ることができます—CSSセレクタを見つけるのに数分しかかかりません。
...
<p>Navi+サポーターとチャットして瞬時に助けを得ることができますが、（1）これによりコアアイデアを理解するのに役立ち、（2）自分でやりたい場合は、以下の指示に従ってください。</p>
...
これはすぐに修正する必要はありませんが、長期的にはNavi+によって作成されたスライドメニューの体験にとって重要です。Navi+が古いスライドメニューを完全に置き換える前に、テーマの元のメニューはまだアクティブです。短い間—約半秒—ユーザーがメニューを読み込み、すぐにハンバーガーボタンをタップした場合（稀なケースですが、可能性はあります）、古いテーマメニューが表示され、視覚的なグリッチを引き起こす可能性があります。
...
<p><strong>スライドメニュー: ローディング効果を追加し、トリガーセレクタをロックする</strong></p>
