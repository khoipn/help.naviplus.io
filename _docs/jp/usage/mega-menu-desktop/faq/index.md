...
#### 質問2: デスクトップメガメニューをモバイルでスライドメニューとして表示するにはどうすればいいですか？

手作業でツリーを再構築する代わりに、**Smart responsive** を使用してください。デスクトップメガメニューを開いて → **Setting → Mobile** → **"Smart responsive"** カードです。このメニューの**ライブリファレンス**をスライドメニューに埋め込みます。ここでのみコンテンツを編集し、モバイル版は自動的に更新されます。
...
**ステップ1: このメニューアイテムテンプレートをコピー** — **Copy embed item** をクリックします。

**ステップ2: ペーストするスライドメニューを選択** — 既存のスライドメニューの **level 1** にペーストします（または、まだスライドメニューがない場合は、まず新しいスライドメニューを作成してください）。

**ステップ3: モバイルコピーからアイテムを非表示** *(オプション)* — 任意のアイテムを開いて → **Visibility** → **"Exclude when embedded"** をオンにします。デスクトップ版には残り、埋め込まれたモバイルコピーからのみ消えます。

マルチカラムレイアウトは自動的にフラット化されます。カラムは1つの縦リストになり、カラムタイトルは削除され、別々のカラムがあった場所に区切り線が挿入されるため、訪問者は2回のタップの代わりに1回のタップですべてのリンクにたどり着けます。

埋め込まれると、カードに **"This menu already shows on mobile"** というメッセージとスライドメニューへの直接リンクが表示されるため、いつでも確認できます。
