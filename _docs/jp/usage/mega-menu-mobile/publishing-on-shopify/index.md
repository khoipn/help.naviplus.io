...
このガイドでは、2つの方法に焦点を当てます: ヘッダーの下にこのメニューを挿入するか、デフォルトのメガメニューを置き換えることです。この方法を使って、他のCSSセレクタを見つけてメニューをトリガーすることもできます。
...
<p>これを行うには、単にウェブサイトのヘッダーのCSSセレクタを特定し、それをテキストボックスに入力します。私たちはすでに、あなたのためにいくつかの人気のあるテーマのCSSセレクタを見つけましたので、以下にリストしています。</p>
<ol>
...
<p>Navi+サポーターとチャットして即時のヘルプを得ることができますが、自分でやる代わりに。\
ただし、(1) これはコアアイデアを理解するのに役立ち、(2) 自分でやりたい場合は、以下の指示に従ってください。</p>
...
この方法は、Shopifyテーマにブロック/セクションを追加することでメニューを挿入します。これは従来のアプローチなので、詳細には説明しません。この方法では、方法1のステップ1で必要なように**「アプリ埋め込みでNavi+をオンにする」**必要すらありません。
