---
description: '**CSSセレクタ**はCSSにおいて非常に興味深い概念です — それはあなたのウェブサイトのどの部分が**置き換え可能**または**Navi+メニューを挿入できる**かを定義します（*前*または*後*のいずれか）。'
lang: jp
layout: default
permalink: /jp/docs/usage/general/find-css-selector/
title: CSSセレクタを見つける
---
# CSSセレクタを見つける

> **Shopify をお使いですか？ このページは不要かもしれません。** Navi+ は **331 の Shopify テーマ**（ハンバーガー、ヘッダー、ロゴ、メインメニュー、検索・カートアイコン）の正しいセレクターをあらかじめ把握しています。Publish ステップ、またはメニュー作成時の「Where should this menu go?」ステップでテーマを選ぶだけで、CSS Selector 欄は自動入力されます。スクリーンショット付きの一覧は [naviplus.io/shopify-themes](https://naviplus.io/shopify-themes/) をご覧ください。以下の手順は、カスタムテーマや一覧にないテーマ、Shopify 以外のサイトの場合のみ必要です。
