...
メニュー項目**Home**を正しい言語で表示するには、次の構文を使用します：
...
これは、**description**および**link-to**フィールドの両方で機能します。

2) Navi+で言語スイッチャーメニューを作成する

Navi+は、メニュー項目から直接言語を切り替える特別な構文をサポートしています。

* **ステップ1：**（オプション）フラグアイコンを追加したい場合は、追加できます。フラグアイコンは、[Flaticon](https://www.flaticon.com "Flaticon")のようなウェブサイトからダウンロードできます（例：<https://www.flaticon.com/authors/flags/rectangular?author_id=1&type=standard>）。
