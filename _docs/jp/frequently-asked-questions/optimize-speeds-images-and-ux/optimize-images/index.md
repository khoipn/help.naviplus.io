...
Navi+は最初に画像を元のサイズでアップロードします。その後、最適化されたアルゴリズムで画像を圧縮し、表示サイズを縮小します。このサイズは、ユーザー体験とSEOの両方にとって理想的であるようにNavi+によって慎重に選ばれます。なぜなら、メニュー内の画像は通常サムネイルやアイコンであり、小さくて迅速に読み込む必要があるからです。

**注意:** ロゴやアイコンのような小さな画像の場合、Navi+は鮮明さを保つために圧縮しません。

その後、Navi+は画像をBunnyCDNに保存します。BunnyCDNは、画像、JavaScript、CSSなどのための強力なコンテンツ配信ネットワーク（CDN）です。BunnyCDNは、グローバルサーバーネットワークを通じて非常に高速な読み込み速度を提供します。これにより、あなたのウェブサイトは遅くなったり、遅延を引き起こしたりすることなく、数百万の訪問を処理できることが保証されます。

#### III. 画像を元の品質のまま保持したい場合はどうすればよいですか？

特別な場合には、Navi+に画像の品質を変更しないようにしたいことがあります。そうしないと、圧縮が期待する品質を低下させる可能性があります。たとえば、重要な製品画像などです。この場合、画像を直接Shopifyにアップロードし、Navi+に元の品質を保持するためのリンクを提供する必要があります。
