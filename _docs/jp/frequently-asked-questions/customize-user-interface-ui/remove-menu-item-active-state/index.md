...
しかし、アクティブ状態は常に必要ではなく、特に他の方法で"あなたのいる場所"を示すことができる広いデスクトップ画面では、時にはメニューをよりクリーンに見せるためにアクティブ状態を削除したい場合があります。
...
方法1：このメニューのみに適用したい場合、CSSコードは次のとおりです（内部スタイルシート/CSSボックスに貼り付けてください）：
...
説明：これは、ページに`#SF-123456789 ul li.navi-active { background: initial; }`のようなCSSルールを追加し、このメニュー（埋め込みID `SF-123456789`）のみに適用され、他のメニューには影響しません。

方法2：すべてのNavi+メニューでアクティブ効果を削除したい場合、次のコードを使用します（グローバルスタイルシート/CSSボックスに貼り付けてください）：
