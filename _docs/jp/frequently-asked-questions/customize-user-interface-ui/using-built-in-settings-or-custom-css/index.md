...
1) メニュー全体の外観を最高レベルで設定するには、標準的なCSSではないが非常にシンプルなスニペットを書くことができます。たとえば、以下のスニペットはメニューの背景を赤に変更します。次のコードを内部スタイルシート / CSSテキストボックスに追加します：
...
2) 各メニューアイテムレベルのサイズ、タイポグラフィ、色を設定します。次のコードを内部スタイルシート / CSSテキストボックスに追加します：
...
3) `font-weight.`を使用してフォントの太さを変更します。次のコードを内部スタイルシート / CSSテキストボックスに追加します：
...
4) 矢印のサイズを調整します。次のコードを内部スタイルシート / CSSテキストボックスに追加します：
