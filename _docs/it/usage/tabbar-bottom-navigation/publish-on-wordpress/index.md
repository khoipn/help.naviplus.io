---
description: Pubblica una Navi+ Tab Bar su WordPress con il plugin Navi+ — senza modifiche al codice. La Tab Bar è un menu fisso, quindi viene visualizzato automaticamente su tutto il sito una volta pubblicato nell'editor.
lang: it
layout: default
...
La **Tab Bar** è un **menu fisso** — si ancorano alla finestra di visualizzazione (tipicamente vicino al fondo su mobile) e rimane in posizione mentre i visitatori scorrono. Su WordPress, il plugin Navi+ rende la Tab Bar visibile su tutto il sito automaticamente; non è necessario incollare alcun codice o shortcode.
...
1. **Installa il plugin** — vedere [Installa il plugin Navi+]({{ site.baseurl }}/docs/integrations/wordpress-woocommerce/install-plugin/).
