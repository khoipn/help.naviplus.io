---
description: '*Ultimo aggiornamento: ottobre 2024*'
lang: it
layout: default
permalink: /it/docs/resources/terms-and-condition/
title: Termini e Condizioni
---
# Termini e Condizioni

*Ultimo aggiornamento: ottobre 2024*

### 1. Introduzione

Benvenuto in **Navi+** (“noi”, “ci”, “nostro”).\ Questi Termini di Servizio (“Termini”) regolano il tuo accesso e utilizzo dell'applicazione, del sito web e dei servizi correlati di Navi+ (collettivamente, il “Servizio”).\ Utilizzando Navi+, accetti questi Termini. Se non sei d'accordo, ti preghiamo di interrompere immediatamente l'uso del Servizio.

Navi+ opera in modo indipendente e lavora su più piattaforme, tra cui **Shopify, WordPress, WooCommerce, Wix, Webflow e Squarespace**.

***

### 2. Pagamento e Sicurezza

Tutti i pagamenti per gli abbonamenti a Navi+ vengono elaborati in modo sicuro tramite piattaforme di terze parti fidate come **Shopify Payments** o **Lemon Squeezy**, a seconda di dove installi l'applicazione.

Queste piattaforme di pagamento garantiscono che ogni transazione sia **sicura, trasparente e protetta** secondo gli standard di sicurezza internazionali.

Navi+ non **memorizza direttamente le informazioni di pagamento** e non elabora rimborsi.\ Effettuando un pagamento, accetti di rispettare i rispettivi **termini e politiche di rimborso** del fornitore di pagamento.

***

### 3. Applicazioni di Terze Parti

Navi+ può includere o integrare software, estensioni o servizi di terze parti.\ Tutte le applicazioni di terze parti utilizzate all'interno di Navi+ aderiscono ai **principi della Licenza MIT** e sono tenute a rispettare tutte le **leggi sul copyright e sulla proprietà intellettuale**.

Sebbene selezioniamo con cura gli strumenti e le librerie che utilizziamo, **Navi+ non è responsabile** per eventuali problemi, danni o perdite di dati causati da applicazioni di terze parti.\ Utilizzando questi strumenti, riconosci che eventuali rischi associati sono sotto la tua responsabilità.

***

### 4. Account e Registrazione

Per utilizzare alcune funzionalità, potrebbe essere necessario creare un account.\ Accetti di:

* Fornire informazioni **accurate, complete e aggiornate**.
* Mantenere le tue **credenziali e password sicure**.
* Essere completamente responsabile per tutte le attività sotto il tuo account.
* Notificare immediatamente Navi+ di qualsiasi accesso non autorizzato o violazione della sicurezza.

Navi+ si riserva il diritto di sospendere o terminare qualsiasi account che appare **compromesso, inattivo o in violazione** di questi Termini o delle leggi applicabili.

***

### 5. Proprietà dei Contenuti e Licenza

Mantieni la piena proprietà di tutti i **contenuti, immagini, dati e materiali** caricati o creati durante l'utilizzo di Navi+.

Utilizzando il Servizio, concedi a Navi+ una **licenza non esclusiva, mondiale e senza diritti d'autore** per memorizzare, elaborare e visualizzare i tuoi contenuti **solo per scopi di funzionamento e miglioramento del Servizio**.

Navi+ non rivendica la proprietà o l'uso dei tuoi contenuti per scopi di marketing, rivendita o qualsiasi scopo non correlato.\ Tutti i diritti rimangono con te.

***

### 6. Limitazione di Responsabilità

Navi+ è fornito “**così com'è**” e “**come disponibile**.”\ Non garantiamo che il Servizio sarà ininterrotto, privo di errori o compatibile con ogni dispositivo o piattaforma.

Navi+ non sarà responsabile per:

* Qualsiasi **danno indiretto, incidentale o consequenziale**, 
* Perdita di entrate, dati o opportunità commerciali, o 
* Problemi tecnici causati da fornitori di terze parti, infrastrutture o azioni degli utenti.

In ogni caso, la responsabilità totale di Navi+ non supererà **l'importo pagato da te** per il Servizio durante i **dodici (12) mesi precedenti**.

***

### 7. Risoluzione del Servizio

Possiamo sospendere o terminare il tuo accesso a Navi+ in qualsiasi momento, senza preavviso, se:

1. Violando questi Termini o le leggi applicabili.
2. Riceviamo una richiesta legittima da Shopify, Lemon Squeezy o qualsiasi piattaforma partner.
3. Il Servizio richiede manutenzione, aggiornamento o interruzione permanente.

Puoi anche terminare il tuo account in qualsiasi momento disinstallando l'applicazione o contattando il supporto.

Dopo la risoluzione, Navi+ può conservare alcuni dati per un periodo limitato per motivi tecnici o legali prima della cancellazione permanente.

***

### 8. Proprietà Intellettuale

Tutti i marchi, design e tecnologie utilizzati all'interno di Navi+ rimangono la **proprietà intellettuale di Navi+ o dei suoi licenziatari**.\ Non puoi copiare, modificare, ridistribuire o rivendere alcuna parte del Servizio senza esplicito consenso scritto.

***

### 9. Giurisdizione e Legge Applicabile

Questi Termini saranno regolati e interpretati secondo le leggi di **Singapore**, senza riguardo ai principi di conflitto di leggi.\ Qualsiasi controversia derivante da o relativa a questi Termini sarà risolta tramite **arbitrato vincolante** a Singapore o in un'altra sede concordata reciprocamente.

***

### 10. Contatto e Supporto

Se riscontri problemi o hai domande, ti preghiamo di contattare il **Navi+ Support Team** — indipendentemente dal tuo piano.\ Valutiamo sinceramente ogni messaggio. Il tuo feedback non solo ci aiuta a risolvere il tuo problema, ma ci aiuta anche a comprendere le esigenze degli utenti e a continuare a migliorare Navi+.

**Email:** <khoipn@naviplus.io>\ **Sito web:** <https://naviplus.io>


---

# Istruzioni per l'Agente: Consultazione di Questa Documentazione

Se hai bisogno di ulteriori informazioni che non sono direttamente disponibili in questa pagina, puoi consultare la documentazione dinamicamente ponendo una domanda.

Esegui una richiesta HTTP GET sull'URL della pagina corrente con il parametro di query `ask`:

```
GET https://help.shopifas.com/manual/website/help.naviplus.io/resources/terms-and-condition.md?ask=<question>
```

La domanda dovrebbe essere specifica, autonoma e scritta in linguaggio naturale.
La risposta conterrà una risposta diretta alla domanda e estratti e fonti pertinenti dalla documentazione.

Utilizza questo meccanismo quando la risposta non è esplicitamente presente nella pagina corrente, hai bisogno di chiarimenti o contesto aggiuntivo, o desideri recuperare sezioni di documentazione correlate.
