---
description: Il s'agit du problème de remplissage en bas sous la Bottom Bar lorsqu'un site Web Shopify utilisant Navi+ est ouvert dans l'application Google sur iPhone.
lang: fr
layout: default
permalink: /fr/docs/limitation/shopify-bottom-bar-issue-on-google-app-iphone/
title: Problème de Bottom Bar Shopify sur l'application Google (iPhone)
---
# Problème de Bottom Bar Shopify sur l'application Google (iPhone)

Il s'agit du problème de remplissage en bas sous la Bottom Bar lorsqu'un site Web Shopify utilisant **Navi+** est ouvert dans l'application Google sur iPhone.

Lorsque vous accédez à un site Web Shopify qui utilise **Navi+** (ou toute application avec un menu en bas fixe, des boutons flottants, etc.), les utilisateurs ouvrant le site Web **dans l'application Google sur iPhone** peuvent parfois voir un espace blanc inhabituel apparaître en bas de l'écran.
...
* Il y a une **Bottom Bar** (Navi+ ou toute autre application), ou un **Floating Action Button (FAB)** tel que des widgets de chat, des boutons de fidélité, etc.
...
* Une Bottom Bar fixe
* Du HTML ou une interface utilisateur générée dynamiquement par JavaScript
* Des mises en page complexes comme Shopify (plusieurs couches de conteneurs et sections en constante évolution)

Lorsque ces conditions se produisent, l'application Google peut mal calculer la hauteur réelle de la fenêtre d'affichage et créer un **faux remplissage en bas**.

➡️ C'est pourquoi le problème apparaît plus fréquemment sur les sites Web Shopify utilisant Navi+ ou d'autres applications de Bottom Bar/FAB.
...
Même si Navi+ est désactivé, d'autres applications Shopify qui affichent des Bottom Bars ou des FAB rencontrent toujours le même problème dans l'application Google.

***

### 4. Preuves que ce problème ne peut pas être entièrement corrigé

* De nombreuses applications Shopify qui incluent une Bottom Bar ou un FAB rencontrent exactement le même problème.
...
* La Bottom Bar s'affiche à la bonne position
...
> "L'application Google sur iPhone a un bug d'affichage lors de l'ouverture de sites Web Shopify qui utilisent une Bottom Bar comme Navi+. Ce problème provient de l'application Google et affecte de nombreuses applications, pas seulement Navi+. Cela ne se produit qu'une fois et disparaîtra après tout tap ou défilement sur l'écran. Si vous utilisez Safari ou Chrome à la place, le problème ne se produira pas."
...
Utilisez ce mécanisme lorsque la réponse n'est pas explicitement présente dans la page actuelle, que vous avez besoin de clarification ou d'informations supplémentaires, ou que vous souhaitez récupérer des sections de documentation connexes.
