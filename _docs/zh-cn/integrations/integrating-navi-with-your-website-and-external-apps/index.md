---
description: 在这里，您将找到有关如何将Navi+与其他应用程序（如聊天工具、忠诚度等）连接的指南。您可以将它们添加为菜单项，直接从Navi+打开应用程序，并确保一切顺利协同工作，而不会在您的网站上重叠。
lang: zh-cn
layout: default
...
在这里，您将找到有关如何将Navi+与其他应用程序（如聊天工具、忠诚度等）连接的指南。您可以将它们添加为菜单项，直接从Navi+打开应用程序，并确保一切顺利协同工作，而不会在您的网站上重叠。
...
• 第1步：使用CSS隐藏粘性应用程序图标。详细信息请参见这里\
• 第2步：使用JavaScript为菜单项，以便单击它们时打开相应的应用程序面板。
...
Navi+还支持您定义类和属性分配（甚至以id="menu-item-1"的形式）以启用与其他应用程序或JavaScript代码的交互。您可以使用类名或属性样式化每个菜单项。
...
使用滑动菜单，除了通过其他Navi+菜单的菜单项打开它们外，您还可以通过调用JavaScript函数：`naviman.openNaviMenu()`来触发Navi+滑动菜单。\
...
* Shopify Inbox：隐藏Shopify Inbox图标并将其集成到Navi+菜单中，作为单击时打开聊天面板的菜单项。详细信息请参见这里
* 打开汉堡菜单或购物车、搜索面板：详细信息请参见这里

### 没有在列表中看到它？

随时向我发送您的集成请求。我很乐意提供帮助，这也支持Navi+的增长。
