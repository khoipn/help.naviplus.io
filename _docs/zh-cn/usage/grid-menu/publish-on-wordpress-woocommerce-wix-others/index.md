...
> **在Wix上？** 查看专用的 [在Wix上发布](/zh-cn/docs/usage/grid-menu/publishing-on-wix/) 指南——从 Wix 应用市场安装 Navi+ 应用，它会自动嵌入，无需代码。下面的手动嵌入仅适用于 Squarespace、Webflow 和自定义网站。

此页面涵盖**手动脚本嵌入**方法，这是适合[Squarespace](https://www.squarespace.com/ "Squarespace")、[Webflow](https://webflow.com/ "Webflow")、[Magento](https://magento-opensource.com/ "Magento")和使用PHP、Node.js或纯HTML构建的自定义网站的正确路径。您有以下实施方法：
