...
> **在WordPress上？** 使用专用的 [Naviplus Menu Builder plugin]({{ site.baseurl }}/docs/usage/fab-floating-menu-quick-access/publish-on-wordpress/) 而不是下面的手动嵌入 — 无需编辑主题，FAB在编辑器中发布后会自动在整个网站上呈现。

> **在Shopify上？** 安装专用的 [Shopify app](https://apps.shopify.com/pronavi-navigation-design) — 它针对该平台进行了优化，具有内置的发布选项。

> **在Wix上？** 查看专用的 [在Wix上发布](/zh-cn/docs/usage/fab-floating-menu-quick-access/publishing-on-wix/) 指南——从 Wix 应用市场安装 Navi+ 应用，它会自动嵌入，无需代码。下面的手动嵌入仅适用于 Squarespace、Webflow 和自定义网站。
...
下面的代码可以在网站上多次使用，具有不同的插入信息（特别是**Embeded ID**，例如SF-123456789）。当网站被渲染时，菜单将在代码插入的位置部署并显示，当条件满足时。此菜单类型适用于：Mega menu、Grid等。
