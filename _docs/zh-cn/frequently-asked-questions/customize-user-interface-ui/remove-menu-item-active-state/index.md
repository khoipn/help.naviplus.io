---
description: 默认情况下，Navi+ 检查当前页面 URL 以在菜单上显示活动状态（使用灰色背景）。这帮助您的客户更轻松地导航。
lang: zh-cn
layout: default
...
默认情况下，Navi+ 检查当前页面 URL 以在菜单上显示活动状态（使用灰色背景）。这帮助您的客户更轻松地导航。
...
解释：这会向页面添加一个 CSS 规则，如 `#SF-123456789 ul li.navi-active { background: initial; }`，仅应用于此菜单（嵌入 ID `SF-123456789`），而不影响其他菜单。

方法 2：如果您想移除所有 Navi+ 菜单的所有活动效果，请使用以下代码（将其粘贴到全局样式表 / CSS 框中）：
