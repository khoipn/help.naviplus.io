---
description: Publish a Navi+ FAB (Floating Action Button) on WordPress with the Navi+ plugin. The FAB is a sticky menu — once published in the editor it appears site-wide automatically.
lang: vi
layout: default
...
A **FAB (Floating Action Button)** is a **sticky** menu — nó nổi trên trang (thường ở một góc) và giữ nguyên khi người dùng cuộn. Trên WordPress, plugin Navi+ tự động hiển thị nó trên toàn bộ trang; bạn không cần dán bất kỳ mã hoặc shortcode nào.
...
1. **Install the plugin** — xem [Install the Navi+ plugin]({{ site.baseurl }}/docs/integrations/wordpress-woocommerce/install-plugin/).
